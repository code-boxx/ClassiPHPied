...
$pALL = [$pBASE, $pAPI, $pLIB, $pBASE."index.foo", $pLIB."GO.foo", $pLIB."classiphpied.sql"];
...
<html>
  <head>
    <title>ClassiPHPied Installation</title>
    <style>
  ...
  <body>
    ...
    <form id="iForm" onsubmit="return install()">
      <div class="iSec">
        ...
        <label>Host (Change this only if wrong, include the path if not deployed in root. E.G. site.com/classiphpied/)</label>
      ...
      <div class="iSec">
        ...
        <label>Name</label>
        <input type="text" name="dbname" required value="classiphpied"/>
        <label>User</label>
...
if ($mode=="D") {
  ...
  // (D3) IMPORT SQL FILE
  try {
    $pdo->exec(file_get_contents($pLIB."classiphpied.sql"));
  } catch (Exception $ex) { exit("Unable to create database - " . $ex->getMessage()); }
