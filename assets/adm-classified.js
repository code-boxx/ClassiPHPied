var cla = {
  ...
  // (B) GO TO PAGE
  //  pg : int, page number
  goToPage : function (pg) { if (pg!=cla.pg) {
    cla.pg = pg;
    cla.list();
  }},
  ...
  del : function (id, confirm) {
    if (confirm) {
      ...
      clp.modal("Please confirm", "Delete this entry?", function(){
        cla.del(id, true);
      });
    }
  }
...
var img = {
  ...
  pick : function (i) {
    ...
    nimg.onclick = function () { img.remove(slot); };
    wrap.innerHTML = "";
  ...
  remove : function (slot) {
    ...
    nbtn.onclick = function () { img.init(slot); };
    wrap.innerHTML = "";
