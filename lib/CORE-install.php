...
define("I_ALL", [
  I_BASE, I_API, I_ASSETS, I_UPLOADS, I_LIB, I_LIB . "CORE-config.php",
  I_LIB . "INSTALL-index.foo", I_LIB . "SQL-classiphpied.sql"
]);

// (A4) DATABASE DEFAULTS
define("I_DB_HOST", "localhost");
define("I_DB_NAME", "classiphpied");
  ...
  <body>
    ...
    <form id="iForm" onsubmit="return install()">
      <div id="iHead">
        ...
        <h1>CLASSIPHPIED INSTALLATION</h1>
      </div>
...
if ($_IMODE=="D") {
  ...
  // (D3) IMPORT SQL FILE
  try {
    $pdo->exec(file_get_contents(I_LIB . "SQL-classiphpied.sql"));
  } catch (Exception $ex) { exit("Unable to create database - " . $ex->getMessage()); }
